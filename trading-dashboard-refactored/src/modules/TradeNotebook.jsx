// modules/TradeNotebook.jsx

import { useState, useEffect, useRef } from "react";
import {
  loadNotebookEntries,
  saveNotebookEntry,
  updateNotebookEntry, // ← MUSS drin sein
  deleteNotebookEntry,
  uploadNotebookScreenshot,
} from "../utils/supabase";

// ─── Subkomponenten (Modul-Ebene) ─────────────────────────────────────────────

function SelBtn({ label, active, color, onClick }) {
  return (
    <button type="button" onClick={onClick} style={{
      padding: "6px 16px", borderRadius: 8,
      border: `1px solid ${active ? color : "#1a1f4a"}`,
...
function Field({ label, children }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      {label && (
        <label style={{ fontSize: 11, color: "#64748b", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em" }}>
          {label}
        </label>
      )}
      {children}
...
function EntryCard({ entry, D, onDelete, onEdit }) {
  ...
  return (
    <div style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 14, overflow: "hidden" }}>
      ...
      >
        <span style={{ fontSize: 12, color: D.textMuted, fontFamily: "monospace" }}>
          {entry.time_entered}
        </span>
        ...
        <span style={{ marginLeft: "auto", color: D.textMuted, fontSize: 12 }}>
          {open ? "▲" : "▼"}
        </span>
      </div>
      ...
      {open && (
        <div style={{
          ...
          {(entry.screenshot_htf_url || entry.screenshot_exec_url) && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              {entry.screenshot_htf_url && (
                <div>
                  <div style={{ fontSize: 10, color: D.textMuted, marginBottom: 6 }}>
                    Daily Bias
                  </div>
                  <img
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {entry.went_good && (
              <div>
                <div style={{ fontSize: 10, color: D.green, marginBottom: 6 }}>
                  What went well
                </div>
                <div style={{ fontSize: 13, color: D.text, whiteSpace: "pre-wrap" }}>
                  {entry.went_good}
                </div>
              </div>
            )}
            ...
            {entry.went_wrong && (
              <div>
                <div style={{ fontSize: 10, color: D.red, marginBottom: 6 }}>
                  What went wrong
                </div>
                <div style={{ fontSize: 13, color: D.text, whiteSpace: "pre-wrap" }}>
                  {entry.went_wrong}
          ...
          {entry.key_takeaway && (
            <div style={{
              background: `${D.purple}15`,
              border: `1px solid ${D.purple}35`,
              borderRadius: 10,
              padding: "12px 16px"
            }}>
              <div style={{ fontSize: 10, color: D.purple, marginBottom: 4 }}>
                Key Takeaway
              </div>
...
const emptyForm = () => ({
  datetime: "", type: "", alongHTF: "",
  wentGood: "", wentWrong: "", keyTakeaway: "",
  fileHTF: null, fileExec: null,
});
...
export default function TradeNotebook({ design }) {
  ...
  const submit = async () => {
    ...
    try {
      let screenshotHTFUrl = null;
      let screenshotExecUrl = null;
      ...
      const payload = {
        ...
        screenshot_htf_url: screenshotHTFUrl,
        screenshot_exec_url: screenshotExecUrl,
      };
  ...
  if (loading) return (
    <div style={{ padding: 48, textAlign: "center", color: D.textMuted, fontSize: 13 }}>Loading…</div>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <button onClick={() => { setShowForm(s => !s); setError(null); }} style={{
          padding: "9px 20px", borderRadius: 10, border: "none",
          background: showForm ? D.border : D.purple,
          color: showForm ? D.textMuted : "#fff",
      ...
      {error && (
        <div style={{ background: `${D.red}15`, border: `1px solid ${D.red}40`, borderRadius: 10, padding: "10px 16px", fontSize: 12, color: D.red }}>
          {error}
        </div>
      )}
        ...
        <div style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 16, padding: 24, display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <Field label="Type">
              <div style={{ display: "flex", gap: 8 }}>
                {["Continuation", "Reversal"].map(t => (
                  <SelBtn key={t} label={t} active={form.type === t}
                    color={t === "Continuation" ? D.green : D.yellow}
                    onClick={() => set("type", t)} />
                ))}
              </div>
            </Field>
            <Field label="Along HTF">
              <div style={{ display: "flex", gap: 8 }}>
                {["Yes", "No"].map(t => (
                  <SelBtn key={t} label={t} active={form.alongHTF === t}
                    color={t === "Yes" ? D.green : D.red}
                    onClick={() => set("alongHTF", t)} />
                ))}
              </div>
            </Field>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <Field label="Daily Bias">
              <AttachButton label="Attach screenshot"
                file={form.fileHTF} onFile={f => set("fileHTF", f)}
                onClear={() => set("fileHTF", null)} uploading={uploadingHTF} D={D} />
            </Field>
            <Field label="Execution">
              <AttachButton label="Attach screenshot"
                file={form.fileExec} onFile={f => set("fileExec", f)}
                onClear={() => set("fileExec", null)} uploading={uploadingExec} D={D} />
            </Field>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <Field label="What went well">
              <Textarea value={form.wentGood} onChange={e => set("wentGood", e.target.value)} D={D} />
            </Field>
            <Field label="What went wrong">
              <Textarea value={form.wentWrong} onChange={e => set("wentWrong", e.target.value)} D={D} />
            </Field>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button onClick={submit} disabled={!form.datetime || saving} style={{
              ...
              background: form.datetime && !saving ? D.purple : D.border,
              color: form.datetime && !saving ? "#fff" : D.textMuted,
